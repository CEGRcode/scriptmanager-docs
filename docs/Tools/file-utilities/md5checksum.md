...
File formats of any kind may be uploaded to the tool since an MD5 sum calculation is agnostic to file formats.

### Output options (TXT)

After pressing "Calculate," the tool will execute to determine a MD5sum value for each input file and save it to a file called `md5checksum.txt` in the "Output Directory." The following is an example of the output format where each line shows the input file within parentheses and the MD5 sum value after the `=`.
